...
func newDBConnection() (*pgxpool.Pool, error) {
	...
	if minConn != "" {
		v, err := strconv.Atoi(minConn) // casting o parse
		if err != nil {
	...
	config, err := pgxpool.ParseConfig(connString) //el parseConfig hace un parce , y valida el conn¡String este correcto
	if err != nil {
